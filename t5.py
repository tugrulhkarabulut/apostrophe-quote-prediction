...
def get_metric_func(label2id, base_classes, tokenizer):
...
def main(cfg):
    ...
    tokenizer = tokenizer = T5TokenizerFast.from_pretrained(cfg.T5.BACKBONE)
    ...
    if cfg.T5.LOAD_DATASET_FROM_DISK and os.path.exists(
        os.path.join(cfg.INPUT, "tokenized_dataset_t5")
    ):
